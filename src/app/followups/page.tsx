...
export default function FollowupsPage() {
  ...
  async function done(id: string) { try { await followUpsApi.complete(id); toast.success('Bajarildi'); load(); } catch {} }
  async function create() {
    if (!form.title.trim() || !form.dueAt) { toast.error('Nomi va vaqt kerak'); return; }
    try { await followUpsApi.create({ ...form, agentId: user?.id }); toast.success('Qo\'shildi'); setShowNew(false); setForm({ title: '', note: '', dueAt: '' }); load(); } catch { toast.error('Xato'); }
  }
    ...
    <CrmLayout>
      <div style={{ padding: 24, maxWidth: 700 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>🔔 Follow-uplar</h1>
        ...
        {showNew && (
          <div style={{ padding: 16, background: 'var(--bg-2)', borderRadius: 12, border: '1px solid var(--border)', marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <input style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--fg)', fontSize: 13 }} value={form.title} onChange={e => setForm(f => ({...f, title: e.target.value}))} placeholder="Nomi *" />
            ...
            <textarea style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--fg)', fontSize: 13, minHeight: 60, resize: 'vertical' }} value={form.note} onChange={e => setForm(f => ({...f, note: e.target.value}))} placeholder="Izoh" />
        ...
        {loading ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--fg-3)' }}>Yuklanmoqda...</div> : (
          <>
            ...
            {items.length === 0 && <div style={{ padding: 60, textAlign: 'center', color: 'var(--fg-3)' }}>Follow-up yo'q 🎉</div>}
          </>
        )}
      </div>
    </CrmLayout>
