...
export default function TasksPage() {
  ...
  async function create() {
    if (!nt.title.trim()) { toast.error('Nomi kerak'); return; }
    try { await tasksApi.create({ ...nt, dueAt: nt.dueAt || undefined, assigneeId: user?.id }); toast.success('Qo\'shildi'); setShowNew(false); setNt({ title: '', dueAt: '', priority: 'MEDIUM' }); load(); } catch { toast.error('Xato'); }
  }
    ...
    <CrmLayout>
      <div style={{ padding: 24, maxWidth: 800 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>☑ Vazifalar</h1>
          <button onClick={() => setShowNew(!showNew)} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#3d7eff', color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>+ Yangi</button>
        </div>
        {showNew && (
          <div style={{ padding: 16, background: 'var(--bg-2)', borderRadius: 12, border: '1px solid var(--border)', marginBottom: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <input style={{ gridColumn: '1/-1', padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--fg)', fontSize: 13 }} value={nt.title} onChange={e => setNt(n => ({ ...n, title: e.target.value }))} placeholder="Nomi *" />
            ...
            <div style={{ gridColumn: '1/-1', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowNew(false)} style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-3)', color: 'var(--fg)', cursor: 'pointer' }}>Bekor</button>
              <button onClick={create} style={{ padding: '7px 14px', borderRadius: 8, border: 'none', background: '#3d7eff', color: 'white', cursor: 'pointer', fontWeight: 700 }}>Saqlash</button>
            </div>
          </div>
        )}
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          {[['','Barchasi'],['TODO','Bajarilmagan'],['IN_PROGRESS','Jarayonda'],['DONE','Bajarilgan']].map(([v,l]) => (
            <button key={v} onClick={() => setSf(v)} style={{ padding: '5px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: sf === v ? '#3d7eff' : 'var(--bg-3)', color: sf === v ? 'white' : 'var(--fg-2)' }}>{l}</button>
          ))}
        </div>
        {loading ? <div style={{ color: 'var(--fg-3)' }}>Yuklanmoqda...</div> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {tasks.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: 'var(--fg-3)' }}>Vazifa yo'q</div>}
            {tasks.map((t: any) => {
                ...
                <div key={t.id} style={{ padding: '12px 16px', background: 'var(--bg-2)', borderRadius: 10, border: '1px solid var(--border)', borderLeft: `3px solid ${PC[t.priority]||'#94a3b8'}`, display: 'flex', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 5 }}>
                      ...
                      <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 6, background: `${PC[t.priority]}20`, color: PC[t.priority]||'#94a3b8' }}>{t.priority}</span>
                    </div>
                  </div>
                </div>
              );
